...
export class MessageHandler {
  ...
  private async handleCaptureEvent(message: any): Promise<void> {
    ...
    await storage.incrementEventCount();
  }
  ...
  private async handleInitializePopup(): Promise<PopupStateMessage> {
    ...
    const eventCount = await storage.get('eventCount');

    // Update trajectory with current event count
    const trajectory = state.currentTrajectory
      ? { ...state.currentTrajectory, event_count: eventCount }
      : null;
  ...
  private async handleStopRecording(): Promise<any> {
    ...
    // Note: Trajectory completion is handled by backend automatically
  ...
  private async handleGetRecordingStatus(): Promise<RecordingStatusMessage> {
    ...
    // Update trajectory with current event count
    const trajectory = state.currentTrajectory
      ? { ...state.currentTrajectory, event_count: state.eventCount }
      : null;
